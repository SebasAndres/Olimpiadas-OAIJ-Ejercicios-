...
bool AllGrid(char color) {
	bool retorno = true;
	for (int j = 0; j < N; j++) {
		for (int i = 0; i < N; i++) { if (C[j][i] != color) { retorno = false; } }
	}
	return retorno;
}
...
list<char> delete_repeated(list<char> original) {
	list <char> nlist;
	for (char c : original) {
		if (find(original.begin(), original.end(), c) == original.end()) { nlist.push_back(c); }
	}
	return nlist;
}

list<char> get_limits(Region r) {
	list<char> limites; char color = r.color;
	for (int *pixl : r.pixeles) {
		int j = pixl[0]; int i = pixl[1];
		// Misma fila
		if (C[f(j + 1)][i] != color) { limites.push_back(C[f(j + 1)][i]); }
		else if (C[f(j - 1)][i] != color) { limites.push_back(C[f(j - 1)][i]); }

		if (C[j, f(i + 1)][i] != color) { limites.push_back(C[j][f(i + 1)]); }
		else if (C[j][f(i - 1)] != color) { limites.push_back(C[j][f(i - 1)]); }
	};
...
void pixeles() {
	...
	int data[2]; int max, contador = 0;
	int min = regiones[0].pixeles.size();
	int bigger, smaller;
	for (Region r: regiones) {
		r.limites = get_limits(r);
		if (r.pixeles.size() >= max) { bigger = contador; max = r.pixeles.size(); }
		else if (r.pixeles.size() <= min) { smaller = contador; min = r.pixeles.size(); }
		data[0] = bigger; data[1] = smaller;
	}
	while (AllGrid('A')) { paint(C, regiones, data); }
}
